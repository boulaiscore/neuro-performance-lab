...
export function ReportActionable({ profile, metrics, aggregates }: ReportActionableProps) {
  ...
  if (systemDiff > 10) balanceInsight = `Fast dominates (+${Math.round(systemDiff)}). Add deep reasoning exercises.`;
  else if (systemDiff < -10) balanceInsight = `Slow dominates (+${Math.round(Math.abs(systemDiff))}). Add reaction drills.`;
  ...
  const leastTrained = [...areas].sort(
    (a, b) => (aggregates.sessionsByArea[a.key] ?? 0) - (aggregates.sessionsByArea[b.key] ?? 0)
  )[0];

  const protocol = [
    weakest.score < 50 ? `Prioritize ${weakest.name} training` : null,
    aggregates.sessionsByArea[leastTrained.key] < 5 ? `Increase ${leastTrained.name} sessions` : null,
    systemDiff > 15 ? "Add 2 slow thinking sessions" : null,
    systemDiff < -15 ? "Add 2 fast thinking drills" : null,
    ...
    <section className="report-page">
      ...
      <div className="actionable-row">
        <div className="actionable-card strength">
          <div className="actionable-card-label">Top Strength</div>
          <div className="actionable-card-value">{strongest.name}</div>
          <div className="actionable-card-score">Score: {Math.round(strongest.score)}</div>
        </div>
        <div className="actionable-card gap">
          <div className="actionable-card-label">Critical Gap</div>
          <div className="actionable-card-value">{weakest.name}</div>
          <div className="actionable-card-score">Score: {Math.round(weakest.score)}</div>
        </div>
      </div>
      ...
      <div className="balance-section">
        ...
        <div className="balance-bar-container">
          <div className="balance-fast" style={{ width: `${fastPct}%` }}>Fast {Math.round(fast)}</div>
          <div className="balance-slow" style={{ width: `${slowPct}%` }}>Slow {Math.round(slow)}</div>
        </div>
